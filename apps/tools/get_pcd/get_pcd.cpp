...
public:
  ...
  ~RingBuffer() {
    for (size_t i = 0; i < blocks_.size(); i++) {
      inno_log_verify(
          blocks_[i].status == kBufFree ||
              (blocks_[i].status == kBufUsing && blocks_[i].written == 0),
    ...
    if (!retry && blocks_[index_].written + length <= kBlockSize) {
      ...
      if (r) {
        ...
        if (blocks_[index_].status != kBufUsed) {
          inno_log_panic("block %d is in wrong status %d", index_,
                         blocks_[index_].status);
        }
        return false;
      }
    }
  ...
  block_t cur_used() { return blocks_[index_used_]; }

  block_t cur_using() { return blocks_[index_]; }
...
private:
  ...
  bool next_free(bool retry = false) {
    ...
    if (blocks_[index_].status != kBufFree) {
      inno_log_warning("All blocks has been filled, the %dth block of memory "
...
public:
  explicit FileRecorder(const std::string &filename, const uint64_t frame_id,
                        bool reflectance, enum FileType file_type)
      : filename_(filename), fd_(-1), point_count_(0),
        reflectance_(reflectance), file_type_(file_type), rosbag_stream_(NULL) {
    if (can_record_bag()) {
      rosbag_stream_ =
          new innovusion::RosbagRecorder(filename.c_str(), NULL, NULL, -1);
  ...
  ~FileRecorder() { recorder_exit_rosbag_(); }
  ...
  void start_writefile_thread(const uint64_t frame_id) {
    std::thread([&]() {
      ...
      fd_ = innovusion::InnoUtils::open_file(
          filename_.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
      ...
      if (is_compact_packet && file_type_ == FILE_TYPE_INNO_PC) {
        ...
        if (inno_lidar_get_anglehv_table(
                g_lidar_handle,
                reinterpret_cast<InnoDataPacket *>(anglehv_table)) == 0) {
          size_t write_size =
              reinterpret_cast<InnoDataPacket *>(anglehv_table)->common.size;
          size_t written = 0;
      ...
      auto write_file = [&](const RingBuffer::block_t &b) {
        ...
        inno_log_verify(written == b.written, "written not match %d/%d",
                        written, b.written);
      };
  ...
  bool can_record_bag() const { return file_type_ == FILE_TYPE_BAG; }
  ...
  void add_data_packet(const InnoDataPacket &pkt_in) {
    ...
    if (file_type_ == FILE_TYPE_INNO_PC) {
      inno_log_verify(CHECK_SPHERE_POINTCLOUD_DATA(pkt_in.type),
                      "cannot convert type=%d to inno_pc", pkt_in.type);
      packet = &pkt_in;
      ...
      if (CHECK_SPHERE_POINTCLOUD_DATA(pkt_in.type)) {
        ...
        if (pkt_in.type == INNO_ROBINW_ITEM_TYPE_COMPACT_POINTCLOUD ||
          ...
          inno_lidar_get_anglehv_table(
              g_lidar_handle,
              reinterpret_cast<InnoDataPacket *>(anglehv_table));
        }
        new_packet = InnoDataPacketUtils::convert_to_xyz_pointcloud_malloced(
            pkt_in, nullptr,
            reinterpret_cast<char *>(
                reinterpret_cast<InnoAngleHVTable *>(
                    (reinterpret_cast<InnoDataPacket *>(anglehv_table)
                         ->payload))
                    ->table));
    ...
    inno_log_verify(
        InnoDataPacketUtils::check_data_packet(*packet, packet->common.size),
        "corrupted pkt");
  ...
  void add_points(const uint64_t frame_id, const double x, const double y,
                  const double z, const uint32_t ref, const uint32_t channel,
                  const uint32_t in_roi, const uint32_t facet,
                  const uint32_t m_ret, const uint32_t confidence_level,
                  const uint32_t flags, const uint32_t elongation,
                  const double timestamp_sec, const uint32_t scanline,
                  const uint32_t scan_idx, const uint32_t is_2nd_return,
    ...
    case FILE_TYPE_PCD:
      ...
      row_format = file_type_ == FILE_TYPE_CSV
                       ...
                       : "%.3f %.3f %.3f %u %u %u %u %u %u %u %u %u "
                         "%.5f %u %u %" PRI_SIZEU " %u\n";
      wri = snprintf(&pcd_buf_[0], kPCDBufSize_, row_format, x, y, z, ref,
                     channel, in_roi, facet, is_2nd_return, m_ret,
                     confidence_level, flags, elongation, timestamp_sec,
  ...
  void add_lite_points(const double x, const double y, const double z,
                       const uint32_t ref, const uint32_t confidence_level,
                       const double timestamp_sec, const uint32_t ring_id) {
    int wri = -1;
    const char *row_format;
    switch (file_type_) {
    case FILE_TYPE_CSV:
    case FILE_TYPE_PCD:
      ...
      row_format = file_type_ == FILE_TYPE_CSV
                       ...
                       : "%.3f %.3f %.3f %u %.5f %u %u\n";
      wri = snprintf(&pcd_buf_[0], kPCDBufSize_, row_format, x, y, z, ref,
                     timestamp_sec, ring_id, confidence_level);
      write_buffer_(&pcd_buf_[0], wri);
      break;
  ...
  void add_en_points(const uint64_t frame_id, const double x, const double y,
                     const double z, const double h_angle, const double v_angle,
    ...
    case FILE_TYPE_PCD:
      ...
      row_format = file_type_ == FILE_TYPE_CSV
                       ...
                       : "%.3f %.3f %.3f %.3f %.3f %u %u %u %u %u %u %u %u %u "
                         "%.5f %u %u %" PRI_SIZEU " %u\n";
      wri = snprintf(&pcd_buf_[0], kPCDBufSize_, row_format, x, y, z, h_angle,
                     v_angle, ref, channel, in_roi, facet, is_2nd_return, m_ret,
                     confidence_level, flags, elongation, timestamp_sec,
                     scanline, scan_idx, frame_id, ring_id);
      write_buffer_(&pcd_buf_[0], wri);
      break;
...
private:
  ...
  void write_dummy_header_() {
    ...
    inno_log_verify(r, "cannot write data to ring buffer, filename is %s",
                    filename_.c_str());
  }

  void rewrite_header_() {
    const char *lite_pcd_header = "# .PCD v.7 - Point Cloud Data file format\n"
                                  "FIELDS "
    ...
    const char *en_pcd_header = "# .PCD v.7 - Point Cloud Data file format\n"
                                "FIELDS "
                                "x y z h_angle v_angle %s channel roi facet "
                                "is_2nd_return multi_return confid_level "
                                "flag elongation timestamp "
                                "scanline scan_idx frame_id ring_id\n"
    ...
    case FILE_TYPE_CSV:
      if (recorder_point_type == POINT_TYPE_EN_PCD) {
        r = snprintf(header_buffer_, sizeof(header_buffer_), csv_header,
                     reflectance_ ? "h_angle,v_angle,reflectance"
                                  : "h_angle,v_angle,intensity");
    ...
    case FILE_TYPE_PCD_BINARY:
      if (recorder_point_type == POINT_TYPE_EN_PCD) {
        r = snprintf(header_buffer_, sizeof(header_buffer_), en_pcd_header,
                     reflectance_ ? "reflectance" : "intensity", point_count_,
                     point_count_,
        ...
        r = snprintf(header_buffer_, sizeof(header_buffer_), pcd_header,
                     reflectance_ ? "reflectance" : "intensity", point_count_,
                     point_count_,
                     file_type_ == FILE_TYPE_PCD ? "ascii" : "binary");
      }
    ...
    inno_log_verify(r < int32_t(sizeof(header_buffer_)),
                    "buffer too small %d vs %" PRI_SIZELU, r,
                    sizeof(header_buffer_));
    inno_log_info("write %" PRI_SIZEU " points to pcd file %s", point_count_,
                  filename_.c_str());
    uint64_t fsize = lseek(fd_, 0, SEEK_END);
    if (point_count_ > 0) {
      inno_log_verify((file_type_ == FILE_TYPE_CSV ? fsize > kMaxCsvSize_
                                                   : fsize > kHeaderSize),
                      "the PCD file size is invalid, fsize: %" PRI_SIZEU
                      ", point_count_ is : %" PRI_SIZEU,
                      fsize, point_count_);
    }
    ...
    inno_log_verify(write_size <= max_write_size,
                    "write_size %" PRI_SIZELU " > header_size %u", write_size,
                    max_write_size);
    uint32_t written = 0;
    while (written < write_size) {
    ...
    inno_log_verify(written == write_size,
                    "written %d,  write_size: %" PRI_SIZELD, written,
                    write_size);
    close(fd_);
    fd_ = -1;
    return;
...
class ExampleProcessor {
public:
  explicit ExampleProcessor(
      const std::string &filename, const int64_t frame_start,
      const int64_t frame_number, const int64_t file_number, int use_xyz_point,
      const std::string &latency_file, int ascii_pcd, int extract_message,
      uint64_t run_time, int lidar_handle, const std::string anglehv_table_file)
      : filename_(filename), frame_start_(frame_start),
        frame_number_(frame_number), file_number_(file_number),
        use_xyz_point_(use_xyz_point), latency_file_(latency_file),
        current_frame_(-1), frame_so_far_(-1), file_so_far_(0),
        file_recorder_(NULL), file_type_(FileRecorder::FILE_TYPE_PCD),
        done_(false), transform_cs_(false), yaw_(0.0), pitch_(0.0), roll_(0.0),
        run_time_(run_time), lidar_handle_(lidar_handle) {
    ...
    if (dot != std::string::npos) {
      ...
      if (strcasecmp(file_extension_.c_str(), ".csv") == 0) {
        ...
        file_type_ = ascii_pcd ? FileRecorder::FILE_TYPE_PCD
    ...
    if (extract_message) {
      ...
      status_fd_ = innovusion::InnoUtils::open_file(
          status_filename_.c_str(), O_WRONLY | O_CREAT | O_APPEND, 0644);
      inno_log_verify(status_fd_ >= 0, "cannot open %s",
                      status_filename_.c_str());
      msg_filename_ = filename_ + "_message_log.txt";
      msg_fd_ = innovusion::InnoUtils::open_file(
          msg_filename_.c_str(), O_WRONLY | O_CREAT | O_APPEND, 0644);
    ...
    if (!latency_file_.empty()) {
      latency_recorder_ =
          new std::fstream(latency_file_, std::ios::out | std::ios::app);
      inno_log_verify(latency_recorder_ && latency_recorder_->is_open(),
                      "cannot open %s save latency", latency_file_.c_str());
    }
    ...
    if (!anglehv_table_file.empty()) {
      ...
      if (ifs.is_open()) {
        ...
        ifs.close();
        inno_log_verify(InnoDataPacketUtils::check_data_packet(
                            *reinterpret_cast<InnoDataPacket *>(anglehv_table_),
                            reinterpret_cast<InnoDataPacket *>(anglehv_table_)
                                ->common.size),
  ...
  void set_need_process_data(int src) { need_process_data_ = src; }
  ...
  void save_message_to_file(const char *message) {
    if (msg_fd_ >= 0) {
      int write_len =
          snprintf(msg_buffer_, sizeof(msg_buffer_), "%s\n", message);
  ...
  static void message_callback_s(const int lidar_handle, void *ctx,
    ...
    reinterpret_cast<ExampleProcessor *>(ctx)->save_message_to_file(
        error_message);
    return;
  }
  ...
  void get_latency_info(const InnoDataPacket &pkt) {
    ...
    *latency_recorder_ << "frame_idx=" << pkt.idx << ","
                       << "latency=" << latency << ","
                       << "ts_start_us="
                       << static_cast<int64_t>(pkt.common.ts_start_us) << ","
  ...
  static int data_callback_s(const int lidar_handle, void *ctx,
    ...
    if (!CHECK_XYZ_POINTCLOUD_DATA(pkt->type) &&
        !CHECK_SPHERE_POINTCLOUD_DATA(pkt->type)) {
    ...
    if (reinterpret_cast<ExampleProcessor *>(ctx)->get_recorder_(
            pkt->idx, pkt->use_reflectance,
    ...
    if (cur_frame_id != pkt->idx) {
      inno_log_info("frame idx:%" PRI_SIZEU ", point_count:%d", cur_frame_id,
                    cur_frame_point);
    ...
    if (reinterpret_cast<ExampleProcessor *>(ctx)->run_time_ > 0) {
      ...
      if (cur_frame_id % 10 == 0 && cur_frame_id != run_time_check_frame_id) {
        ...
        if (now_time - start_time >
            reinterpret_cast<ExampleProcessor *>(ctx)->run_time_ * 1000) {
          inno_log_info(
              "get_pcd start_time: %" PRI_SIZEU " ms, end_time: %" PRI_SIZEU
              " ms, run_time:%" PRI_SIZEU " ms",
              start_time, now_time,
              reinterpret_cast<ExampleProcessor *>(ctx)->run_time_ * 1000);
          reinterpret_cast<ExampleProcessor *>(ctx)->set_done();
        }
...
private:
  inline FileRecorder *get_recorder_(const int64_t frame_id,
    ...
    if (filename_.size() == 0) {
      ...
      // do nothing
    } else if ((frame_so_far_ < frame_start_ + frame_number_ * file_number_) ||
               run_time_ > 0) {
      // in record range
      if ((frame_so_far_ >= frame_start_ + frame_number_ * file_so_far_) &&
          run_time_ == 0) {
        if (file_recorder_) {
          delete file_recorder_;
          file_recorder_ = NULL;
      ...
      if (file_recorder_ == NULL) {
        ...
        if (file_number_ != 1 || ::filename_with_timestamp) {
          if (::filename_with_timestamp) {
            ...
            fn = filename_base_ + tmp + "_" + std::to_string(timestamp) +
                 file_extension_;
          } else {
            fn = filename_base_ + "-" + std::to_string(file_so_far_) +
                 file_extension_;
          }
        }
        file_recorder_ =
            new FileRecorder(fn, frame_id, reflectance, file_type_);
        file_so_far_++;
  ...
  void process_data_(const InnoDataPacket &pkt) {
    ...
    } else if ((filename_.size() == 0) || (frame_number_ == -1) ||
               (run_time_ > 0) ||
  ...
  void sensor_to_world_cs(double *new_x, double *new_y, double *new_z, double x,
  ...
  void recoder_xyz_point(const InnoDataPacket &pkt, double frame_timestamp_sec,
                         FileRecorder *recorder) {
    const InnoXyzPoint *points =
        reinterpret_cast<const InnoXyzPoint *>(pkt.payload);
    double x, y, z;
    for (uint32_t i = 0; i < pkt.item_number; i++) {
      ...
      recorder->add_points(
          pkt.idx, x, y, z, pt.refl, pt.channel, pt.in_roi, pt.facet,
          pt.multi_return, pkt.confidence_level, pt.type, pt.elongation,
          frame_timestamp_sec + pt.ts_10us / k10UsInSecond, pt.scan_id,
          pt.scan_idx, pt.is_2nd_return, pt.ring_id);
    }
  }

  void recoder_en_xyz_point(const InnoDataPacket &pkt,
                            double frame_timestamp_sec,
                            FileRecorder *recorder) {
    const InnoEnXyzPoint *points =
        reinterpret_cast<const InnoEnXyzPoint *>(pkt.payload);
    double x, y, z;
    for (uint32_t i = 0; i < pkt.item_number; i++) {
      ...
      recorder->add_points(pkt.idx, x, y, z,
                           pkt.use_reflectance ? pt.reflectance : pt.intensity,
                           pt.channel, pt.in_roi, pt.facet, pt.multi_return,
                           pkt.confidence_level, (pt.firing << 2), 0,
                           frame_timestamp_sec + pt.ts_10us / k10UsInSecond,
                           pt.scan_id, pt.scan_idx, pt.is_2nd_return, 0);
    }
  }

  void process_data_xyz_point_(const InnoDataPacket &pkt) {
    inno_log_verify(CHECK_XYZ_POINTCLOUD_DATA(pkt.type), "invalid packet %u",
                    pkt.type);
    ...
    FileRecorder *recorder =
        get_recorder_(pkt.idx, pkt.use_reflectance, frame_timestamp_sec);
    if (recorder) {
  ...
  void en_block_to_en_xyz_point(const InnoDataPacket &pkt,
                                FileRecorder *recorder, int unit_size,
                                int return_number) {
    const InnoEnBlock *blocks =
        reinterpret_cast<const InnoEnBlock *>(pkt.payload);
    double frame_timestamp_sec = pkt.common.ts_start_us / kUsInSecond;
    const InnoEnBlock *block =
        reinterpret_cast<const InnoEnBlock *>(&blocks[0]);

    for (uint32_t i = 0; i < pkt.item_number;
         i++, block = reinterpret_cast<const InnoEnBlock *>(
      ...
      for (uint32_t channel = 0; channel < kInnoChannelNumber; channel++) {
        for (uint32_t m = 0; m < return_number; m++) {
          const InnoEnChannelPoint &pt =
              block->points[innoblock_get_idx(channel, m)];
          InnoXyzrD xyzr;
          uint32_t scan_id = 0;
          if (pt.radius > 0) {
            if (pkt.type == INNO_FALCONII_DOT_1_ITEM_TYPE_SPHERE_POINTCLOUD) {
              ...
              scan_id = channel_mapping[index] +
                        block->header.facet * tdc_channel_number;
              InnoDataPacketUtils::get_xyzr_meter(
            ...
            h_angle = static_cast<double>(full_angles.angles[channel].h_angle) /
                      kInnoAngleUnitPerDegree;
            v_angle = static_cast<double>(full_angles.angles[channel].v_angle) /
                      kInnoAngleUnitPerDegree;

            if (transform_cs_) {
              sensor_to_world_cs(&x, &y, &z, xyzr.x, xyzr.y, xyzr.z, roll_,
                                 pitch_, yaw_);
            }

            if (recorder_point_type == POINT_TYPE_EN_PCD) {
              recorder->add_en_points(
                  pkt.idx, x, y, z, h_angle, v_angle,
                  pkt.use_reflectance ? pt.reflectance : pt.intensity, channel,
                  block->header.in_roi, block->header.facet, m,
                  pkt.confidence_level, pt.type | (pt.firing << 2),
                  pt.elongation,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  scan_id, block->header.scan_idx, pt.is_2nd_return, 0);
            } else if (recorder_point_type == POINT_TYPE_LITE_PCD) {
              recorder->add_lite_points(
                  x, y, z, pkt.use_reflectance ? pt.reflectance : pt.intensity,
  ...
  void co_block_to_en_xyz_point(const InnoDataPacket &pkt,
                                FileRecorder *recorder, int unit_size,
                                int return_number) {
    const InnoCoBlock *blocks =
        reinterpret_cast<const InnoCoBlock *>(pkt.payload);
    double frame_timestamp_sec = pkt.common.ts_start_us / kUsInSecond;
    const InnoCoBlock *block =
        reinterpret_cast<const InnoCoBlock *>(&blocks[0]);
    int ret = 0;
    if (anglehv_table_ == nullptr) {
      anglehv_table_ = new char[kInnoAngleHVTableMaxSize];
      ret = inno_lidar_get_anglehv_table(
          lidar_handle_, reinterpret_cast<InnoDataPacket *>(anglehv_table_));
    }

    inno_log_verify(ret == 0, "get anglehv_table failed");
    for (uint32_t i = 0; i < pkt.item_number;
         i++, block = reinterpret_cast<const InnoCoBlock *>(
                  reinterpret_cast<const char *>(block) + unit_size)) {
      // calculate (x,y,z) cartesian coordinate from spherical coordinate
      // for each point in the block
      // 1. use get_full_angles() to restore angle for each channel
      ...
      InnoDataPacketUtils::get_block_full_angles(
          ...
          reinterpret_cast<char *>(
              reinterpret_cast<InnoAngleHVTable *>(
                  (reinterpret_cast<InnoDataPacket *>(anglehv_table_)->payload))
                  ->table));
      double x, y, z, h_angle, v_angle;

      for (uint32_t channel = 0; channel < kInnoCompactChannelNumber;
           channel++) {
        for (uint32_t m = 0; m < return_number; m++) {
          const InnoCoChannelPoint &pt =
              block->points[innocoblock_get_idx(channel, m)];
          InnoXyzrD xyzr;
          uint32_t scan_id = 0;
          if (pt.radius > 0 && InnoDataPacketUtils::is_robinw_inside_fov_point(
            ...
            if (recorder_point_type == POINT_TYPE_EN_PCD) {
              recorder->add_en_points(
                  pkt.idx, x, y, z, h_angle, v_angle, pt.refl, channel,
                  block->header.in_roi, block->header.facet, m,
                  pkt.confidence_level, pt.firing << 2, 0,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  scan_id, block->header.scan_idx, pt.is_2nd_return, scan_id);
            } else if (recorder_point_type == POINT_TYPE_LITE_PCD) {
              recorder->add_lite_points(
                  x, y, z, pt.refl, pkt.confidence_level,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  scan_id);
            } else {
              recorder->add_points(
                  pkt.idx, x, y, z, pt.refl, channel, block->header.in_roi,
                  block->header.facet, m, pkt.confidence_level, pt.firing << 2,
                  0,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  scan_id, block->header.scan_idx, pt.is_2nd_return, scan_id);
            }
  ...
  void block_to_xyz_point(const InnoDataPacket &pkt, FileRecorder *recorder,
                          int unit_size, int return_number) {
      ...
      for (uint32_t channel = 0; channel < kInnoChannelNumber; channel++) {
        for (uint32_t m = 0; m < return_number; m++) {
          ...
          if (pt.radius > 0) {
            ...
            int ring_id = inno_lidar_get_ring_id(
                lidar_handle_,
                static_cast<InnoLidarMode>(pkt.common.lidar_mode),
            ...
            if (recorder_point_type == POINT_TYPE_EN_PCD) {
              recorder->add_en_points(
                  ...
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  block->header.scan_id, block->header.scan_idx,
                  pt.is_2nd_return, ring_id);
            } else if (recorder_point_type == POINT_TYPE_LITE_PCD) {
              recorder->add_lite_points(
                  x, y, z, pt.refl, pkt.confidence_level,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
              ...
              recorder->add_points(
                  pkt.idx, x, y, z, pt.refl, channel, block->header.in_roi,
                  block->header.facet, m, pkt.confidence_level, pt.type,
                  pt.elongation,
                  frame_timestamp_sec + block->header.ts_10us / k10UsInSecond,
                  block->header.scan_id, block->header.scan_idx,
                  pt.is_2nd_return, ring_id);
  ...
  */
  void process_data_cpoint_(const InnoDataPacket &pkt) {
    ...
    InnoDataPacketUtils::get_block_size_and_number_return(pkt, &unit_size,
                                                          &return_number);
        ...
        for (auto i = 0; i < checkout_count; i++) {
          if (receive_datacallback_.load()) {
            ...
            if (i == checkout_count - 1) {
              done_ = true;
              inno_log_error("datacallback function did not receive data for "
                             "more than 10s, app exiting");
              return;
            }
          }
        }
...
void usage(const char *arg0) {
  inno_fprintf(
      ...
      "\t\t0 = Lite PCD (XYZICRT:Contains X,Y,Z,Intensity,Confidence,Ring "
      "ID,and Timestamp)\n"
      "\t\t1 = Normal PCD (Includes additional information such as scan_id and "
      "Scan_idx)\n"
      "\t\t2 = Enhanced PCD (Includes all Normal PCD features plus Horizontal "
      "and Vertical Angle)]\n"
      "\t[--version]\n",
      arg0);
  inno_fprintf(2,
...
int main(int argc, char **argv) {
  ...
  if (inno_pc_filename.size() > 0) {
    ...
    inno_log_verify(pcap_filename.length() < sizeof(param.pcap_param.filename),
                    "%s length > %" PRI_SIZET, pcap_filename.c_str(),
                    sizeof(param.pcap_param.filename));
    strncpy(param.pcap_param.filename, pcap_filename.c_str(),
            pcap_filename.length() + 1);
    ...
    handle = inno_lidar_open_live("live", /* name of lidar instance */
                                  ...
                                  use_tcp ? INNO_LIDAR_PROTOCOL_PCS_TCP
   ...
   ***********************/
  ExampleProcessor processor(filename, frame_start, frame_number, file_number,
                             use_xyz_point, latency_file, ascii_pcd,
                             extract_message, run_time, handle,
                             anghv_table_file);
  ...
  if (use_xyz_point == 1) {
    ...
    inno_log_verify(false, "invalid use_xyz_point number, "
                           "must in [0, 1, 2]");
  }
  if (force_vehicle_coordinate) {
   ...
   ***********************/
  ret = inno_lidar_set_config_name_value(
      handle, "LidarClient_StageClientRead/misorder_correct_enable", "1");
  ...
  if (inno_pc_filename.size() == 0 && pcap_filename.size() == 0) {
    ret = inno_lidar_set_config_name_value(
        handle, "LidarClient_Communication/get_conn_timeout_sec", "5.0");
    inno_log_verify(ret == 0,
                    "inno_lidar_set_config_name_value"
   ...
   ***********************/
  ret = inno_lidar_set_callbacks(handle, processor.message_callback_s,
  ...
  if (use_ring_id_convert) {
    ...
    if (ret != 0) {
      inno_log_warning("get ring id converter failed. Please ignore this if "
                       "you needn't ring id.");
    } else {
      void *converter = inno_lidar_get_ring_id_converter(handle);
      if (converter) {
        processor.set_ring_id_converter(
            reinterpret_cast<RingIdConverterInterface *>(converter));
      }
    }
  }
  /***********************
  ...
  while (!processor.is_done()) {
    ...
    if (max_distance) {
      inno_lidar_set_attribute_string(handle, "vehicle_speed",
                                      std::to_string(vehicle_speed).c_str());
    }
  }
